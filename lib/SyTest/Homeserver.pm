...
sub _get_dbconfigs
{
   ...
   foreach my $db ( keys %db_configs ) {
      ...
      # $db_config->{driver} is a reference to the `driver` field in the configuration
      ...
      my $db_driver = $db_config->{driver};
      if( defined $db_driver ) {
         if( ($db_driver eq 'psycopg2') || ($db_driver eq 'psycopg') ) {
            $db_config->{type} = 'pg';
         }
         elsif( $db_driver eq 'sqlite3' ) {
            $db_config->{type} = 'sqlite';
         }
         else {
            die "Unrecognised DB name '$db_driver'";
         }
      }
